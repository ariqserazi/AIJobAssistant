...
- **Google Sheet ID**: `YOUR_GOOGLE_SHEET_ID` | Worksheet: `Sheet1`.
...
| :--- | :--- | :--- |
| **Full Name** | Jane Doe (configured in config.json) | Configurable in `config.json` |
| **Email** | `candidate_email@example.com` | Primary contact & OTP receiver |
| **Phone** | `555-123-4567` | Formats: `5551234567`, `(555) 123-4567` |
| **Location** | New York, New York | Select: `New York, NY` or `New Jersey` |
...
| **Undergrad Degree** | State University | BS Computer Science, May 2024 (Summa Cum Laude, GPA 3.86) |
| **Graduate Degree** | State University | MS Computer Science, Projected May 2028 (Currently enrolled / Masters) |
| **Graduation Range** | Projected May 2028 | Matches `2028`, `Spring 2028`, `Jan - April 2028`, `2027 & later` |
| **University Restrictions** | Strictly Enrolled University | If asked "Are you enrolled at Northeastern / Columbia / NYU / etc.?", strictly answer **NO** |
...
- Service account: `~/.config/gcloud/legacy_credentials/your-service-account@your-project.iam.gserviceaccount.com/adc.json`.
...
2. **`application_engine/queue_workday.json`**:
   ...
   - Standard credentials: `candidate_email@example.com` | `YourWorkdayPassword123!#`.
...
python3 -c "
import gspread, os
gc = gspread.service_account(os.path.expanduser('~/.config/gcloud/legacy_credentials/your-service-account@your-project.iam.gserviceaccount.com/adc.json'))
...
Whenever candidate requests to find internships or jobs:
...
2. **Harvest fresh daily postings**: Run the automated discovery harvester:
   ...
   ```
3. **Automated Deduplication**: The harvester fetches live markdown tables from SimplifyJobs (Summer 2027, Summer 2026, New Grad) and Pitt CSC, checks each role against all 678+ confirmed rows in Google Sheet `YOUR_GOOGLE_SHEET_ID`, and skips any role previously applied (by both URL and normalized company/title).
